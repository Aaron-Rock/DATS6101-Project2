...
Relevant: 
Our dataset is relevant with e-commerce shopping being so popular with consumers. Nowadays, e-commerce companies heavily invest in AB testing, and the data/models provided from this project could give more insight into the direction that these companies would like to proceed in, in order to maximize their gains/appropriate metric.

Time-oriented: 
We will perform quick exploratory data analysis the first half to gain a better understanding of the new dataset we have chosen, and then for the second half, build models for prediction.
...
Plotting highlights the imbalance between positive and negative classes in our response variable (Revenue). Also distribution is right skewed. In the histogram of weekend we can see Revenue is more over weekdays rather than weekends.
