...
dtree_model <- rpart(Revenue ~ . - Region, data=df, method="class", control = list(maxdepth = 8, cp = 0.01))
printcp(dtree_model)
```
...
```{r, results='markup'}
loadPkg("caret") 
cm = confusionMatrix(predict(dtree_model, type = "class"), reference = dtree_model[, "Revenue"] )
```
